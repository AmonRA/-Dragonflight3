...
function minimap:CreateTopPanel(cluster)
    ...
    local zoneText = DF.ui.Font(panel, 11, 'Zone', {1, 0.82, 0}, 'LEFT')
    zoneText:SetPoint('LEFT', panel, 'LEFT', 11, 0)

    panel:SetScript('OnEnter', function()
        local hour, minute = GetGameTime()
        -- DF.lib.ShowSimpleTooltip(panel, string.format('Server Time: %02d:%02d', hour, minute))
    end)
...
function minimap:CreateZoomButtons(cluster)
    ...
    zoomIn:SetScript('OnEnter', function()
        -- DF.lib.ShowSimpleTooltip(zoomIn, 'Zoom In')
    end)
    ...
    zoomOut:SetScript('OnEnter', function()
        -- DF.lib.ShowSimpleTooltip(zoomOut, 'Zoom Out')
    end)
...
function DF.lib.CreateGameTimeButton(parent)
    ...
    button:SetScript('OnEnter', function()
        local hour, minute = GetGameTime()
        -- DF.lib.ShowSimpleTooltip(button, string.format('Game Time: %02d:%02d', hour, minute))
    end)
